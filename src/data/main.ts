...
function composeFirstFloor(horizontal: FloorPlan, lower: FloorPlan): FloorPlan {
  ...
  const exteriorWalls = [...(upper.exteriorWalls ?? []), ...(lowerFragment.exteriorWalls ?? [])];
  ...
  return {
    ...
    walls: [...upper.walls, ...lowerFragment.walls],
    ...
    notes: [
      {
        ...
        at: [900, 1040],
        label: { kk: 'Сол қанаттың ішкі жоспары жоқ', ru: 'Внутренний план левого крыла отсутствует' },
      },
    ],
    rooms: [...upper.rooms, ...lowerFragment.rooms],
  };
}

function composeUpperFloor(horizontal: FloorPlan, lower: FloorPlan, upperOrigin: [number, number], upperScale: number, lowerOrigin: [number, number], lowerScale: number): FloorPlan {
  const upper = transformFragment(horizontal, (x, y) => [
    729 + (y - upperOrigin[1]) * upperScale,
    35 + (upperOrigin[0] - x) * upperScale,
  ]);
  const bottom = transformFragment(lower, (x, y) => [
    729 + (x - lowerOrigin[0]) * lowerScale,
    1128 + (y - lowerOrigin[1]) * lowerScale,
  ]);
  return {
    ...
    sourceFiles: [...horizontal.sourceFiles, ...lower.sourceFiles],
    viewBox: [680, 0, 740, 1170],
    outline: `${upper.outline} ${bottom.outline}`,
...
export const mainPlans: FloorPlan[] = [
  firstFloor,
  composeUpperFloor(mainSourcePlans[1], mainSourcePlans[5], [829, 323], 177 / 154, [205, 688], 645 / 697),
  composeUpperFloor(mainSourcePlans[2], mainSourcePlans[6], [807, 327], 177 / 130, [313, 683], 645 / 596),
  mainFloor4,
];
